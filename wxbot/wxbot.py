...
class WXBot:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        #print "ORG MSG -------------------: \n",json.dumps(msg),"\n---------------------"
        if mtype == 1:
            msg_content['url'] = msg["Url"]
            sub_mtype = msg["SubMsgType"]
            if sub_mtype == 48: #位置
                msg_content['type'] = 1
            ...
            obj_data = self.session.get(msg_content['data'])
            msg_content['img'] = obj_data.content.encode('hex')
            msg_content['content-type'] = obj_data.headers["Content-Type"]
        elif mtype == 34:
            #语音消息
            msg_content['type'] = 4
            msg_content['data'] = self.get_voice_url(msg_id)
            obj_data = self.session.get(msg_content['data'])
            msg_content['voice'] = obj_data.content.encode('hex')
            msg_content['content-type'] = obj_data.headers["Content-Type"]
            msg_content['VoiceLength'] = msg['VoiceLength'] #语音的长度
        elif mtype == 42:
            msg_content['type'] = 5
    ...
    def update_wx_remarkname(self, contact, remark_name):
        """
        更新微信联系人的备注信息
            同时修改account_info对应的contact信息
        """
        self.account_info["normal_member"][contact["UserName"]]["RemarkName"] = remark_name
        ...
        try:
            ...
            print "---------- update_wx_remarkname failed ------------"
            print e.message, traceback.format_exc()
            return False
        
    
    def handle_contact_new(self, contact, iter):
        """
        处理新增用户列表
        contact:
            完整的微信contact信息 （utf-8编码的）
        
        """
        pass
    
    def handle_group_new(self, contact, iter):
        """
        处理新增群
        contact:
            完整的微信群信息 （utf-8编码的）
    ...
    def handle_msg(self, r):
        ...
        #处理新增用户
        for ocnt in r['ModContactList']:
            if ocnt['UserName'].find('@@') != -1:  #新加的群
                #通知应用去处理新增好友
                iter = len(self.group_list)
                self.group_list.append(ocnt)
                self.account_info['normal_member'][ocnt['UserName']] = {'type': 'group', 'info': ocnt}
                self.handle_group_new(ocnt, iter)
                continue
            elif ocnt['VerifyFlag'] & 8 != 0:  # 公众号
                continue
            elif ocnt['UserName'] in special_users:  # 特殊账户
                continue
            elif ocnt['UserName'] == self.my_account['UserName']: # 自己
                continue
            
            #新加的联系人
            if self.account_info['normal_member'].has_key(ocnt['UserName']) == True:
                continue
            iter = len(self.contact_list)
            self.contact_list.append(ocnt)
            self.account_info['normal_member'][ocnt['UserName']] = {'type': 'contact', 'info': ocnt}
            #通知应用去处理新增好友
            self.handle_contact_new(ocnt, iter)
        ...
        for msg in r['AddMsgList']:
            ...
            message = {'msg_type_id': msg_type_id,
                       ...
                       'user': user}
            self.handle_msg_all(message, msg)
    ...
    '''
        deprecated
        发送分享消息(待处理)
            web暂时不支持发送分享消息，等待协议破解
    '''
    def send_share_msg(self, msg):
        url = self.base_uri + '/webwxsendmsg?pass_ticket=%s' % self.pass_ticket
        msg_id = str(int(time.time() * 1000)) + str(random.random())[:5].replace('.', '')
        params = {
            'BaseRequest': self.base_request,
    ...
    '''
        ...
        发送语音消息(待处理)
            web暂时不支持发送语音，等待协议破解
    '''
    def send_audio_msg(self, url, dst):
        url = self.base_uri + '/webwxsendmsg?pass_ticket=%s' % self.pass_ticket
        msg_id = str(int(time.time() * 1000)) + str(random.random())[:5].replace('.', '')
        params = {
            'BaseRequest': self.base_request,
    ...
    def upload_media(self, fpath, is_img=False):
        ...
        try:
            ...
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                ...
                if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                    print '[ERROR 2] Upload media failure, what = ',r.text
                    sys.stdout.flush()
                    return None
            mid = json.loads(r.text)['MediaId']
            return mid
        except Exception,e:
            print "---------- upload_media failed ------------"
            print e.message, traceback.format_exc()
            return None
    ...
    def send_file_msg_by_uid(self, fpath, uid):
        ...
        try:
            ...
            print "---------- send_file_msg_by_uid failed ------------"
            print e.message
            return False
    ...
    def send_img_msg_by_uid(self, fpath, uid):
        ...
        try:
            ...
            print "---------- send_img_msg_by_uid failed ------------"
            print e.message, traceback.format_exc()
            return False
    ...
    '''
    通过好友验证,添加对方为好友
        适用于免验证情况下，别人添加自己后，自己同步添加别人
    '''
    def verify_user(self, contact, txt):
        #url = self.base_uri + '/webwxverifyuser?sid=%s&skey=%s&lang=en_US&pass_ticket=%s&r=%s520' \
        #                      % (self.sid, self.skey, self.pass_ticket, int(time.time()))
        url = self.base_uri + '/webwxverifyuser?r=%s560' % (int(time.time()))
        data = {
                'BaseRequest': self.base_request,
                'Opcode':2,
    ...
    def sync_check(self):
        ...
        url = 'https://' + self.sync_host + '.wx2.qq.com/cgi-bin/mmwebwx-bin/synccheck?' + urllib.urlencode(params)
        try:
