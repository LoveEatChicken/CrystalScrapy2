...
func (this *IdManager) AllocID(outId OutIdType) (InIdType, error) {
	...
	if outId == 0 {
		return 0, errors.New("illegal outId [0]")
	}

	if this.idStatus.CurId >= this.idStatus.MaxInId {
		return 0, u.ErrFmt("InId [%d] out of limit MaxInId[%d]", this.idStatus.CurId, this.idStatus.MaxInId)
	}
...
func (this *IdManager) GetOutID(inId InIdType) (OutIdType, error) {
	...
	if inId >= this.idStatus.MaxInId {
		return 0, u.ErrFmt("inId [%d] illegal MaxInId[%d]", inId, this.idStatus.MaxInId)
	}
