...
func (this *ValueManager) Open(path string) error {
	...
	for i := 0; uint32(i) < this.fileCnt; i++ {
		...
		if err != nil {
			return u.ErrFmt("open mfile[%d],szie[%d] fail[%s]", i, sz, err.Error())
		}
	}
...
func (this *ValueManager) WriteValue(inId InIdType, v Value) error {
	if inId > this.valueStatus.MaxInId {
		return u.ErrFmt("inId [%d] illegal MaxInId[%d]", inId, this.valueStatus.MaxInId)
	}
	...
	if fileNo >= this.fileCnt {
		return errors.New("inId out of limit")
	}
...
func (this *ValueManager) ReadValue(inId InIdType) (Value, error) {
	if inId > this.valueStatus.MaxInId {
		return nil, u.ErrFmt("inId [%d] illegal MaxInId[%d]", inId, this.valueStatus.MaxInId)
	}
	...
	if fileNo >= this.fileCnt {
		return nil, errors.New("inId out of limit")
	}
