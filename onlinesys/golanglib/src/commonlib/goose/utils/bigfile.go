...
func (this *BigFileIndex) Decode(buf []byte) error {
	...
	if len(buf) < 9 {
		return errors.New(fmt.Sprintf("buf length [%d] error", len(buf)))
	}
...
func (this *BigFile) Open(path string, name string) error {
	...
	if this.bigfileStat.SuggestFileSize == 0 {
		return errors.New("BigFile.Open stat file error")
	}
	...
	if this.bigfileStat.FileCnt > 0 {
		...
		if sz != int64(this.bigfileStat.LastFileOffset) {
			return errors.New(fmt.Sprintf("FileStatInfo Error LastFileOffset:[%d] != FileSize:[%d]",
...
func (this *BigFile) Append(buf []byte) (*BigFileIndex, error) {
	...
	if i.Offset != this.bigfileStat.LastFileOffset {
		return nil, errors.New(fmt.Sprintf("getOffset[%d] LastFileOffset[%d]",
			i.Offset, this.bigfileStat.LastFileOffset))
	}

	n, err := f.Write(buf)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("BigFile.Append write fail : %s", err.Error()))
	}
	if uint32(n) != i.Length {
		// 写成功,但是写入长度跟期望对不上
		// 回滚文件指针
		this.readwriteFile.Seek(int64(i.Offset), 0)
		return nil, errors.New(fmt.Sprintf("write succ but length error : %s", err.Error()))
	}
	// 更新状态文件
...
func (this *BigFile) Read(i BigFileIndex, desBuf []byte) error {
	if i.FileNo >= this.bigfileStat.FileCnt {
		return errors.New(fmt.Sprintf("BigFile.Read FileNo[%d] Error", i.FileNo))
	}
	if i.Length > uint32(len(desBuf)) {
		return errors.New(fmt.Sprintf("BigFileIndex.Length[%d] > len(desBuf)[%d]",
	...
	if uint32(n) != i.Length {
		return errors.New(fmt.Sprintf("offset[%d] destBuf len[%d],ReadAt len[%d]",
			i.Offset, i.Length, n))
	}
	if err != nil {
		return err
...
func (this *BigFile) openRwFile(fileno uint8) error {
	...
	if this.fileModel == bigFileModelInit {
		...
		return errors.New(fmt.Sprintf("File Status Error : [%d]", this.fileModel))
	}
	if err != nil {
		return err
	}
