...
type StyIndexer struct {
	...
	// 全匹配检索的字段
	wholeMatchField []string

	// 按title进行处理的字段
	titleMatchField []string
}
...
func (this *StyIndexer) ParseDoc(doc interface{}, context *goose.StyContext) (
	...
	if err != nil {
		...
		return 0, nil, nil, nil, u.ErrFmt("extract doc as json fail")
	}
	...
	DocId, err := u.StringSignInt32(doc_id)
	if err != nil {
		log.Llog.Error(err.Error())
		return 0, nil, nil, nil, u.ErrFmt("doc_id sign to int32 fail")
	}
	...
	// write termInDoc
	termList, err = this.extractTerm(document)
	log.Llog.Info("termCount:%d", len(termList))

	// write value
	value, err = this.extractValue(document)
	if err != nil {
		log.Llog.Warn("extractValue fail : %s", err)
		return
	}
...
func (this *StyIndexer) Init(conf config.Conf) (err error) {
	...
	if len(this.valueBoost) != 1 {
		log.Llog.Error("Strategy.ValueBoost length must be 4,see extractValue")
		return errors.New("read conf fail")
	}

	// 指定有哪些字段要做全匹配检索
	this.wholeMatchField = append(this.wholeMatchField, "brand")

	this.titleMatchField = append(this.titleMatchField, "name")
